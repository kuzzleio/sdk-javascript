...
Given(
  ...
  function (id) {
    return this.kuzzle.document
      .delete(this.index, this.collection, id)
      .then((content) => {
...
Given("the collection has a document with id {string}", function (id) {
  return this.kuzzle.document
    .create(this.index, this.collection, { a: "document" }, id, {
      refresh: "wait_for",
    })
    .then((content) => {
      this.content = content;
...
When("I check if {string} exists", function (id) {
  return this.kuzzle.document
    .exists(this.index, this.collection, id)
    .then((content) => {
      this.content = content;
    });
});
...
When("I count how many documents there is in the collection", function () {
  return this.kuzzle.document
    .count(this.index, this.collection, {})
    .then((content) => {
      this.content = content;
    });
});
...
When("I create a document in {string}", function (collection) {
  return this.kuzzle.document
    .create(this.index, collection, { a: "document" }, "some-id", {
      refresh: "wait_for",
    })
    .then((content) => {
      this.content = content;
...
When("I delete the documents [{string}, {string}]", function (id1, id2) {
  ...
  return this.kuzzle.document
    .mDelete(this.index, this.collection, [id1, id2], { refresh: "wait_for" })
    .then((content) => {
      this.content = content;
    })
    .catch((error) => {
...
When("I get documents [{string}, {string}]", function (id1, id2) {
  this.ids = [id1, id2];
  return this.kuzzle.document
    .mGet(this.index, this.collection, [id1, id2])
    .then((content) => {
      this.content = content;
    });
});
...
When("I search a document with id {string}", function (id) {
  return this.kuzzle.document
    .search(this.index, this.collection, {
      query: {
        match: {
          _id: id,
        },
...
When(
  ...
  function (query, from, size) {
    return this.kuzzle.document
      .search(this.index, this.collection, JSON.parse(query), { from, size })
      .then((content) => {
        this.content = content;
      });
  }
...
When("I search the next documents", function () {
  return this.content.next().then((content) => {
    this.content = content;
  });
});
...
When("I update a document with id {string}", function (id) {
  ...
  return this.kuzzle.document
    .update(this.index, this.collection, id, { some: "update" })
    .then((content) => {
      this.content = content;
    })
    .catch((err) => {
...
When(
  ...
  function (id, key, val) {
    return this.kuzzle.document
      .update(this.index, this.collection, id, { [key]: val })
      .then((content) => {
        this.content = content;
      });
  }
...
Then("the document {string} should be created", function (id) {
  return this.kuzzle.document
    .get(this.index, this.collection, id)
