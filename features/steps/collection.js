...
Given("it has a collection {string}", function (collection) {
  return this.kuzzle.collection.create(this.index, collection).then(() => {
    this.collection = collection;
  });
});

Given("I truncate the collection {string}", function (collection) {
  return this.kuzzle.collection.truncate(this.index, collection, {
    refresh: "wait_for",
  });
});
...
When(
  ...
  function (collection, withMapping) {
    const mapping = withMapping
      ? { properties: { gordon: { type: "keyword" } } }
      : undefined;

    return this.kuzzle.collection
      .create(this.index, collection, mapping)
      .then((content) => {
        this.content = content;
      })
      .catch((error) => {
...
When("I delete the specifications of {string}", function (collection) {
  return this.kuzzle.collection
    .deleteSpecifications(this.index, collection)
    .catch((error) => {
      this.error = error;
    });
});
...
Then("the mapping of {string} should be updated", function (collection) {
  return this.kuzzle.collection
    .getMapping(this.index, collection)
    .then((mappings) => {
      should(mappings).eql({
        dynamic: "true",
        properties: {
...
Then("the specifications of {string} should be updated", function (collection) {
  return this.kuzzle.collection
    .getSpecifications(this.index, collection)
    .then((specifications) => {
