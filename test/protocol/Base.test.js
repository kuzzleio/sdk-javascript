...
const {
  PendingRequest,
} = require("../../src/protocols/abstract/PendingRequest");

describe("Common Protocol", () => {
  let sendSpy, protocol;
  ...
  describe("#query", () => {
    ...
    it("should reject if not ready", () => {
      ...
      return protocol
        .query({ controller: "foo", action: "bar" })
        .then(() => {
    ...
    it("should emit the request when asked to", () => {
      ...
      should(sendSpy).be.calledOnce().be.calledWith(request);
    });
    ...
    it('should fire a "queryError" event and reject if an error occurred', () => {
      ...
      return protocol
        .query({ requestId: "foobar", response: response })
    ...
    it("should resolve the promise once a response has been received", () => {
      const response = { result: "foo", error: null, status: 42 },
        request = { requestId: "someEvent", response: response };

      return protocol.query(request).then((res) => {
  ...
  describe("#clear", () => {
    it("should discard and reject cleared requests", () => {
      const request1 = { requestId: "12345", body: "foobar" },
      ...
      const pending1 = protocol._pendingRequests.get(request1.requestId),
      ...
      return pending1.promise.should.be
        .rejectedWith({
          message:
