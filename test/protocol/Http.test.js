  ...
  describe("#connect", () => {
    ...
    it("should fallback to server:info if server:publicApi is not available", () => {
      protocol._sendHttpRequest
        .onCall(0)
        .resolves({ error: { status: 404 } })
        .onCall(1)
        .resolves({ result: serverInfoResponse });
    ...
    it("should inject customRoutes", () => {
      ...
      protocol._warn = sinon.stub();
      protocol._sendHttpRequest = sinon
        .stub()
        .resolves({ result: serverPublicApiResult });
    ...
    it("should initialize protocol status and route list", () => {
      ...
      return promise.then(() => {
        ...
        should(protocol.routes.foo.bar).match({
          verb: "VERB",
          url: "/foo/bar",
        });
